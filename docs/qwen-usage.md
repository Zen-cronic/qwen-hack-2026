...
| **Qwen-TTS** | native `…/multimodal-generation/generation` | `qwen3-tts-flash` | per-shot narration with a **cast** — the agent names a speaker, the server casts a voice from a probed roster; `voice` required and part of the cache key |
