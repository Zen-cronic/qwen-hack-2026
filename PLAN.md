...
- `specs.py` — pydantic ShotSpec/Assertion/AssertionResult; **closed vocabulary, exactly 9 assertion types** (compiler rejects invented ones): `duration_between`, `brightness_range`, `flicker_below`, `scene_cuts`, `camera_motion`, `palette_deltae` (Tier A) · `subject_present` (Tier 0/B) · `identity_consistent`, `action_completed` (Tier B, advisory).
- `compiler.py` — pack YAML defaults + LLM dynamic assertions → validated per-shot lists.
- `tier0.py` — 1 t2i still/shot (`wan2.1-t2i-plus`) + static checks BEFORE any video spend (1/25th cost).
